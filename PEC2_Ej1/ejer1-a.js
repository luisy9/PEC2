...
const findOne = (list, { key, value }, { onSuccess, onError }) => {
  //Generamos un setTimeout que parar cuando hayan pasado 2000 milisegundos
  setTimeout(() => {
    //Luego hacemos una busqueda por el inidice de el objeto que le pasemos
    const element = list.find((element) => element[key] === value);
    //Esta linea quiere decir que si element es true, es decir que ha encontrado el inidice encontes se ejecutara onSuccess(element), y le pasaremos true
...
/* Creamos funcion de onError para imprimir por consola un mensaje
    Esta funcion solo se ejecutara no ha encontrado el indice 
*/
const onError = ({ msg }) => console.log(msg);

//Creamos un un array con objetos dentro.
const users = [
  {
    name: 'Carlos',
    rol: 'Teacher',
